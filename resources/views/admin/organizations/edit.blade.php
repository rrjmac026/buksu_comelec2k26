<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Organization</h2>
                <p class="text-sm text-violet-600 dark:text-violet-400 mt-0.5">Updating: <strong>{{ $organization->name }}</strong></p>
            </div>
        </div>
    </x-slot>
                    ...
                    <div>
                        ...
                        <select name="college_id"
                            ...
                            @foreach(\App\Models\College::orderBy('name')->get() as $college)
                                <option value="{{ $college->college_id }}" {{ old('college_id', $organization->college_id) == $college->college_id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea name="description" placeholder="This organization is..."
                                  ...
                                  class="w-full px-4 py-2.5 rounded-xl text-sm
                                         ...
                                         focus:outline-none focus:ring-2 focus:ring-violet-400
                                         placeholder-violet-300 dark:placeholder-violet-600
                                         @error('description') border-rose-400 @enderror">{{ old('description', $organization->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-rose-500 flex items-center gap-1"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p>
                        @enderror
                    </div>
