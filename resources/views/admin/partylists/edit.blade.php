<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100 leading-tight">Edit Partylist</h2>
                <p class="text-sm text-violet-600 dark:text-violet-400 mt-0.5">Updating: <strong>{{ $partylist->name }}</strong></p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-2xl mx-auto">
        <form method="POST" action="{{ route('admin.partylists.update', $partylist) }}" class="space-y-5">
            @csrf @method('PUT')
                ...
                <div class="p-6 space-y-5">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
                            Partylist Name <span class="text-rose-500">*</span>
                        </label>
                        <input type="text" name="name" value="{{ old('name', $partylist->name) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                                  ...
                                  class="w-full px-4 py-2.5 rounded-xl text-sm
                                         ...
                                         focus:outline-none focus:ring-2 focus:ring-violet-400
                                         placeholder-violet-300 dark:placeholder-violet-600
                                         @error('description') border-rose-400 @enderror">{{ old('description', $partylist->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-rose-500 flex items-center gap-1"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <p class="text-xs text-violet-500 dark:text-violet-400 flex items-center gap-1">
                        <i class="fas fa-info-circle"></i> Partylist name must be unique
                    </p>
                </div>
            </div>
