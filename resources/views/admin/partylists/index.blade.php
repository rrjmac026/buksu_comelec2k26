<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-xl text-gray-800 dark:text-gray-100 leading-tight">Partylist Management</h2>
                <p class="text-sm text-violet-600 dark:text-violet-400 mt-0.5">Manage all registered partylists</p>
            </div>
            <a href="{{ route('admin.partylists.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white
                ...
                <i class="fas fa-plus text-xs"></i> Add Partylist
            </a>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        @foreach([
            ['label' => 'Total Partylists',    'value' => $total,            'icon' => 'fa-flag',          'color' => 'from-violet-600 to-violet-400'],
            ['label' => 'Total Candidates',   'value' => $totalCandidates,  'icon' => 'fa-user-tie',      'color' => 'from-emerald-600 to-emerald-400'],
            ['label' => 'Total Votes',        'value' => $totalVotes,       'icon' => 'fa-check-double',  'color' => 'from-sky-600 to-sky-400'],
            ['label' => 'Avg Candidates/List','value' => $avgCandidates,    'icon' => 'fa-chart-column',  'color' => 'from-rose-600 to-rose-400'],
        ] as $stat)
    ...
    <div class="bg-white dark:bg-violet-950/40 rounded-2xl border border-violet-100 dark:border-violet-800/50 shadow-sm p-4 mb-4">
        <form method="GET" action="{{ route('admin.partylists.index') }}" class="flex flex-wrap gap-3 items-end">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-violet-100 dark:border-violet-800/50
                        ...
                        <th class="text-left px-5 py-3.5 text-xs font-bold text-violet-600 dark:text-violet-400 uppercase tracking-wider">Partylist</th>
                        <th class="text-left px-5 py-3.5 text-xs font-bold text-violet-600 dark:text-violet-400 uppercase tracking-wider hidden md:table-cell">Description</th>
                ...
                <tbody class="divide-y divide-violet-50 dark:divide-violet-800/30">
                    ...
                    <tr class="hover:bg-violet-50/40 dark:hover:bg-violet-900/20 transition-colors duration-150">
                        ...
                        <td class="px-5 py-3.5">
                            <a href="{{ route('admin.partylists.show', $partylist) }}"
                               class="flex items-center gap-3 hover:opacity-80 transition">
                                ...
                                <div>
                                    <div class="font-semibold text-gray-800 dark:text-gray-100">{{ $partylist->name }}</div>
                                </div>
                            </a>
                        </td>
                        ...
                        <td class="px-5 py-3.5 hidden md:table-cell">
                            <span class="text-gray-600 dark:text-gray-400 line-clamp-1">
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold
                                ...
                                <i class="fas fa-user-tie text-xs"></i> {{ $partylist->candidates_count ?? 0 }}
                            </span>
                        </td>
                        ...
                        <td class="px-5 py-3.5">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.partylists.destroy', $partylist) }}"
                                      style="display:inline-block;"
                                      onsubmit="return confirm('Are you sure? This cannot be undone.') && ({{ $partylist->candidates_count ?? 0 }} === 0 || confirm('This partylist has candidates. Are you sure?'));">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold
                    ...
                    <tr>
                        <td colspan="4" class="px-5 py-8 text-center">
                            <div class="flex flex-col items-center justify-center gap-2">
                                ...
                                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">No partylists found.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
