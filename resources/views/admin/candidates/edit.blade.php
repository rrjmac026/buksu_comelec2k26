...
.ac-input.err, .ac-select.err { border-color:rgba(248,113,113,0.5); }
.ac-select {
...
.ac-photo-label { font-size:0.72rem; font-weight:600; color:rgba(255,251,240,0.7); }
.ac-photo-sub   { font-size:0.62rem; color:rgba(249,180,15,0.4); margin-top:2px; }
...
.ac-btn-primary { background:linear-gradient(135deg,#f9b40f,#fcd558); color:#380041; box-shadow:0 3px 12px rgba(249,180,15,0.3); }
.ac-btn-primary:hover { transform:translateY(-1px); box-shadow:0 5px 18px rgba(249,180,15,0.45); color:#380041; }
.ac-btn-ghost { background:transparent; border:1px solid rgba(249,180,15,0.2); color:rgba(249,180,15,0.55); }
...
<div class="ac-wrap">
    ...
    <div class="ac-page-header">
        ...
        <div>
            <div class="ac-page-title">Edit Candidate</div>
            <div class="ac-page-sub">Updating: <strong>{{ $candidate->first_name }} {{ $candidate->last_name }}</strong></div>
        </div>
    </div>
            ...
            <div class="ac-card-body" style="display:flex;flex-direction:column;gap:18px;">
                <div>
                    ...
                    <textarea name="platform" rows="4" class="ac-textarea"
                              placeholder="Enter candidate's campaign platform and promises…">{{ old('platform', $candidate->platform) }}</textarea>
                </div>
                <div>
                    ...
                    <div class="ac-photo-preview">
                        <img src="{{ asset('images/candidates/' . $candidate->photo) }}"
                             alt="{{ $candidate->first_name }}" class="ac-photo-thumb">
                        <div>
                            <div class="ac-photo-label">Current Photo</div>
                            <div class="ac-photo-sub">Upload a new file below to replace it</div>
                        </div>
                    </div>
                    @endif
